...
const CHANGELOG := [
	"v0.7.93 - Build per MAC. Aggiunto l'export macOS alla pipeline: artefatto 'world-order-macos' (un .zip con world-order.app), build UNIVERSALE che gira sia su Mac Intel sia su Apple Silicon (M1/M2/M3). L'app non e' firmata (build personale), quindi al primo avvio macOS la blocca: si sblocca una volta sola con 'xattr -dr com.apple.quarantine world-order.app' oppure click destro -> Apri. Guida in docs/multiplayer-lan.md. Stato build native: Desktop (Linux/Windows/macOS) pronte; Android (APK) ancora in lavorazione.",
