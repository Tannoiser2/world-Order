...
const VERSION := "v0.7.92"
const CHANGELOG := [
	"v0.7.92 - APP NATIVE per la LAN: aggiunta la pipeline che genera le build native (Linux, Windows e APK Android) scaricabili da GitHub Actions. Servono perche' il multiplayer LAN funziona SOLO tra app native (dal browser non si puo' ospitare). Come si gioca: tutti sulla stessa rete, uno fa Ospita (LAN) e mostra il suo IP, gli altri si Uniscono con quell'IP, poi l'host preme Avvia. Anche un telefono Android puo' fare da host. Guida in docs/multiplayer-lan.md.",
