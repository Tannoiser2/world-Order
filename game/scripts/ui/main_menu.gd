...
const CHANGELOG := [
	...
	"v0.7.121 - FIX regole: la fase di PREPARAZIONE ora c'e' anche al Round 1. Prima il Round 1 saltava tutto (Focus Domestic forzato, dritti in Azione), quindi mancava la fase dove si girano (ready) le carte Nazione esaurite, si puo' aumentare la Produzione e si produce a seconda del Focus scelto. Ora ogni giocatore, anche al Round 1, sceglie il Focus toccando una colonna sulla plancia e ne applica gli effetti (ready + Produzione del Focus + aumento Produzione opzionale) prima di passare alla fase Azione. +1 test (verify_round1_prep).",
