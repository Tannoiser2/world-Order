...
const VERSION := "v0.7.167"
const CHANGELOG := [
	"v0.7.167 - 3 fix su segnalazioni: 1) La sezione diagnostica di rete (visibile solo in partita online) era un riquadro fisso sovrapposto alla plancia - ora è una sezione COLLASSABILE dentro il Registro, con un bottone 'Copia' per incollarla altrove quando serve segnalare un blocco. 2) Commercio: con una vendita/acquisto parziale già piazzato su una risorsa, la casella corrispondente alla quantità di partenza mostrava '-0' in rosso come se fosse un costo - ora mostra uno '0' neutro (nessun guadagno/costo, è solo il ripristino della quantità iniziale). 3) Commercio: tenendo premuta un'icona di risorsa appariva il riquadro grigio di default invece dell'evidenziazione dorata già usata per la risorsa selezionata - corretto. +2 test nuovi (casella '0' nel Commercio, Get a Growth Card avvolto in una scelta - anche come 2a carta nello stesso turno, via rete). Suite motore 140/140, sweep completo di tutti gli 84 test verify_*.gd senza nuove regressioni.",
	"v0.7.166 - Ottimizzazione UI: meno popup a elenco testuale, più interazione diretta sulla plancia/mano - stesso principio già usato per Commercio/Aumento Produzione. 1) 'Aiuti Economici e Militari' e 'Collaborazione con gli Alleati' (esaurire una Nazione Alleata) ora si fanno toccando DIRETTAMENTE la carta sulla plancia (le caselle idonee si evidenziano), riusando la stessa infrastruttura già esistente di Invest/Build a Base, invece di leggere i nomi in un popup. 2) 'Vantaggio Operativo' (attivazione gratuita di un Asset Strategico posseduto) si fa ora toccando la carta Asset già mostrata in mano, invece di un popup con i nomi ripetuti in testo - 'Salta' resta disponibile nella barra scelte. +2 test estesi, +1 test di rete nuovo (client sceglie/attiva, l'host applica e ribroadcast). Suite motore 140/140, sweep completo di tutti gli 82 test verify_*.gd senza nuove regressioni.",
	"v0.7.165 - La build Web (quella pubblicata su GitHub Pages) ora si può usare OFFLINE e installare come app: abilitata la Progressive Web App nell'export (era predisposta ma disattivata), con le icone del gioco (144/180/512px, generate dall'icona esistente) e un colore di sfondo coerente. Dopo la prima apertura online, il browser mette in cache tutto il necessario (motore, dati, carte) e le aperture successive funzionano anche senza connessione - utile per giocare in Hot Seat o contro i Bot senza rete. Su iPad/iPhone (Safari) e Android si può fare 'Aggiungi alla schermata Home' per ottenere un'icona a schermo intero come una vera app. La modalità in RETE resta ovviamente esclusa offline (serve una connessione per forza).",
	"v0.7.164 - AUDIT rete: dopo l'unificazione dell'Aumento Produzione gratuito da carta (v0.7.163), trovato che toccare la casella evidenziata mutava la Produzione DIRETTAMENTE in locale invece di passare dal command bus, come ogni altra azione condivisa (mappa/plancia) - se il giocatore di turno era un CLIENT (non l'host), la scelta non arrivava mai all'host (autorità): restava invisibile agli altri e veniva persa al prossimo aggiornamento di stato dall'host. Corretto con un nuovo comando di rete dedicato ('free_increase_pick'), stessa architettura di 'increase_production'/pick_region & co. +1 test di rete nuovo (client sceglie, l'host applica e ribroadcast; l'host non può scegliere al posto del client di turno) che riproduce il bug sul codice precedente e conferma il fix. Verificato anche che il resto del layout ridimensionabile recente (splitter board/mappa, mano, linguette potenze) è correttamente locale a ogni schermo e non necessita sync di rete. Suite motore 140/140, sweep completo verify_*.gd (81 test) senza nuove regressioni.",
	"v0.7.163 - 3 fix su segnalazioni: 1) NUOVI ACCORDI FINANZIARI (carta Azione, investi in una Nazione Alleata altrui con IDE) non contava mai come un vero Invest per chi la giocava - mancava l'aggiornamento dell'IDE proprio, quindi non si vedeva mai il Return on Investments (2x valore ogni round) né i bonus/Obiettivi che contano le Country con IDE, nonostante il costo pagato. Corretto (con guardia contro il doppio IDE sulla stessa Country). 2) Il tasto 'Fine turno' non si riusciva più a premere (finiva sotto uno degli splitter trascinabili recenti) - spostato nella TOP BAR, coerente con le linguette delle potenze ormai anch'esse nel pannello Board. 3) L'Aumento Produzione GRATUITO da una carta ('Aumenta N Produzioni' - Growth Industrial Development, Market Boost Production, China Rapid Industrialization) usava un elenco di bottoni nella barra scelte, disomogeneo rispetto al passo Aumento Produzione del Focus (che evidenzia le caselle sulla plancia) - unificato: ora usa la STESSA interfaccia (caselle evidenziate, tocca per scegliere), meno codice duplicato. +2 test estesi, +1 test nuovo. Suite motore 140/140, sweep completo verify_*.gd senza nuove regressioni.",
	"v0.7.162 - Due richieste sul nuovo layout ridimensionabile: 1) Le linguette (bandiere) delle potenze, prima una barra a sé in fondo allo schermo, sono ora DENTRO il pannello Board (in cima), sempre visibili/ispezionabili qualunque board sia mostrata o quanto sia largo lo splitter board/mappa. 2) La MANO ha ora una sua barra divisoria trascinabile (come quella board/mappa) per scegliere quanto e' alta quando aperta, invece di un'altezza fissa calcolata dalla dimensione delle carte - continua a chiudersi da sola quando non serve (Commercio/Produce/scelte sulla mappa). Corretto anche un difetto visivo introdotto dallo splitter board/mappa: quando la mano si espande molto, lo splitter (e il Registro) ora si fermano davvero al suo bordo superiore invece di disegnarsi sopra le carte. +1 test (verify_board_tabs_and_hand_resize, 5 check). Suite motore 140/140.",
	"v0.7.161 - Aggiunta una barra divisoria TRASCINABILE fra il pannello Board (plancia) e la mappa: prima la proporzione fra le due colonne era fissa (calcolata automaticamente in base al rapporto della mappa) - ora si trascina il confine per allargare l'una o l'altra a piacere, e il contenuto (mappa, plancia, Registro) si riadatta dinamicamente in tempo reale. +1 test (verify_board_splitter, 6 check). Suite motore 140/140.",
	"v0.7.160 - Grossa raffica di fix su segnalazioni: 1) STRENGTHEN ALLIANCE non permetteva di costruire una 2a Base nella stessa Country come descritto dalla carta (il modifier 'base_repeat_once' era nei dati ma nessuna parte del motore lo leggeva mai) - corretto, con limite a 2 Basi totali per Country. 2) REGISTRO rifatto: ora è una colonna VERA (la mappa si restringe, non ci finisce più sotto come un overlay), righe colorate per Nazione e in NERETTO per le azioni, font più piccolo, ordine invertito (più recenti in alto), cambi di turno/round con un'intestazione ben visibile (non più testo con simboli deboli). 3) L'Aumento Produzione del Focus DOMESTICO ora vale per QUALSIASI delle 7 Produzioni (non solo Energia/Materie/Cibo, che restano l'unica scelta per errore) - Diplomatico/Militare restano specifici alla propria risorsa, come da Player Board. 4) La Produce del Focus Domestico (Beni di consumo + Servizi, senza limite di conteggio) avvisa ora se non sono stati ancora scelti TUTTI i tipi consentiti, non solo il caso 'conta N tipi' delle carte. 5) L'Aumento Produzione ora EVIDENZIA la casella giusta direttamente sulla plancia (come Commercio/Produce) invece di un elenco di bottoni testuali. 6) Le Growth Card sono ora in RIGA (non più impilate, meno spazio) e l'evidenziazione 'usabile' abbraccia solo la carta, non l'intera riga. 7) Obiettivi Superpotenze: confermato che il motore già sommava i VP di ENTRAMBI gli Obiettivi al round 3 e di nuovo al round 6 (nessuno viene scartato) - il riepilogo di fine round ora lo rende ESPLICITO mostrando il contributo di ciascun Obiettivo (prima un unico numero già sommato dava l'impressione che ne contasse solo uno). 8) Quando una carta con più azioni in sequenza (es. 'Produci poi Get a Growth Card') fallisce a META' (nessuna Growth acquistabile dopo aver speso le risorse nel Produce), ora appare un AVVISO PROMINENTE che spiega che il resto della carta è già stato applicato, invece di una riga di stato silenziosa che dava l'impressione che il turno fosse finito senza motivo. 9) La MANO resta visibile durante la scelta del Focus (prima spariva per tutta la Preparazione) - utile per decidere quale Focus scegliere. 10) Corrette le etichette mancanti nelle scelte a bottoni (es. 'discard' mostrato grezzo in inglese invece di 'Scarta N carte') e un bug latente: la carta IN RISOLUZIONE poteva finire proposta tra le carte da scartare/eliminare di se stessa. +7 test nuovi/estesi. Suite motore 140/140, sweep completo verify_*.gd senza nuove regressioni.",
	"v0.7.159 - FIX su segnalazione Produce: 1) le Armate del Focus Militare non hanno una casella stampata sulla resource track (vanno in riserva) - la v0.7.157 le aveva messe in una striscia di trascinamento INVENTATA sulla plancia, poco chiara/scoperta ('non si capisce come costruire Carri, non si può trascinare il segnalino'). Tornate a un ± CHIARO con etichetta 'N/Produzione' nella barra scelte in alto, come prima della v0.7.157 ma stavolta a quantità assoluta (non delta) - il costo in Cibo+Materie Prime restava corretto anche prima, solo invisibile per lo stesso motivo. 2) 'Produci 3 tipi' (Ordine Esecutivo/carte): la selezione multi-tipo funzionava già a dovere, ma non c'era alcun avviso se si confermava avendone scelto solo 1 su 3 - il bottone 'Conferma' ora mostra 'Conferma (solo N/M tipi)' finché non sono stati scelti tutti i tipi consentiti, così non si conclude per sbaglio l'azione a metà. +test (verify_produce_ui_real: barra Armate ± via rendering reale, avviso 'solo N/M tipi' via rendering reale). Suite motore 138/138.",
	"v0.7.158 - Ricontrollate le carte scansionate dell'espansione: le 32 carte Obiettivo e le 12 carte Orientamento Strategico in italiano (Diplomacy & Dominance) avevano tutte una striscia bianca residua in alto (ritagliate un filo troppo in alto dal foglio originale) - ririttagliate pulite dai PDF originali (Obiettivi.pdf, Orientamenti Strategici nuovi.pdf). Pannello Obiettivi: rimossa l'etichetta 'Obiettivi' (le carte si riconoscono da sole) e carte leggermente più piccole. Confermato che le condizioni/PV mostrati sotto ogni carta Obiettivo sono ricalcolati IN TEMPO REALE ad ogni aggiornamento (non un valore fisso calcolato una sola volta). +1 test (verify_objectives_panel_ui, 4 check). Suite motore 138/138.",
	"v0.7.157 - BUG SERIO trovato e corretto: in Europa, Asia Centrale ed Asia Orientale-Pacifico i PV di maggioranza scattavano dopo un solo VERO piazzamento di Influenza, perché i cubetti INIZIALI di setup (riga colorata sopra la traccia stampata) venivano messi negli stessi slot 'permanenti' usati per decidere se la Regione può segnare — bastava completare quel poco che restava. Ora i cubetti iniziali contano per la maggioranza e danno 1 PV di presenza (come da regolamento) ma NON occupano più uno slot permanente vero: la Regione segna solo quando i giocatori riempiono DAVVERO tutta la riga permanente stampata sul tabellone (verificato su assets/board/board.jpg). Il 'fix' precedente per MENA/South Asia (raddoppiare gli slot con caselle finte) non corrispondeva al tabellone reale — ripristinati ai conteggi giusti (MENA 3, non 6; South Asia 2, non 4). Corretta anche la ricetta di produzione delle risorse derivate (segnalata sbagliata): Beni di consumo = Materie Prime + Energia (invariata), Servizi = Cibo + Energia (prima Cibo+Materie), Armate = Cibo + Materie Prime (prima solo Materie), Diplomazia si paga in denaro (prima Cibo). Rifatta la UI di Produzione: niente più testo '+N -N' sulle caselle né bottoni ± per le Armate — si TRASCINA il segnalino della risorsa lungo la sua track fino allo slot desiderato (drag&drop nativo, come il Commercio) o lo si tocca; le risorse primarie necessarie si vedono scalare in diretta sulla loro track. Ordine Esecutivo spostato nella mano (dopo le carte Strategiche): si seleziona una carta e si tocca l'Ordine Esecutivo per attivarlo (la carta è il costo, come per gli Asset Strategici), invece del vecchio bottone separato. Verificato che 'Ottimizzazione delle Entrate' funziona correttamente (ora con un messaggio visibile quando si attiva) e che il bonus PV per Strategic Asset/Executive Order NON usati è già implementato. +4 test (verify_majority_setup esteso, verify_produce_drag, verify_growth_ottimizzazione_entrate, verify_executive_order riscritto). Suite motore 138/138.",
	"v0.7.156 - FIX su segnalazione: 1) Selezionare una Nazione Alleata per lo sconto (Impegnati/Migliora Relazioni) ora la fa girare SUBITO in anteprima (grigia), come le Nazioni davvero esaurite - prima l'animazione non partiva finche' non si confermava. 2) La barra dello sconto ora ha un SOLO bottone 'Conferma' (rimosso 'Salta (nessuno sconto)': girare 0 carte equivale gia' a nessuno sconto). 3) Le Growth Card 1x/round (es. 'Riduzione Burocrazia') ora si illuminano quando usabili e si clicca DIRETTAMENTE la carta per attivarle (si gira in grigio con l'animazione) - non serve piu' un bottone 'Usa' separato. 4) Choose Focus: la sequenza era nell'ordine SBAGLIATO (si produceva PRIMA e si aumentava la Produzione DOPO, quindi l'aumento non contava mai per la produzione dello stesso turno). Ora e' Pronte (riattiva le Nazioni, animato) -> Aumento Produzione (opzionale) -> Produzione del Focus (ultimo passo, stessa Produce UI delle carte): il +1 risorsa immediato dell'Aumento e' davvero spendibile nella Produzione dello stesso turno. +2 test (verify_exhaust_ui 7 check, verify_focus_increase_produce_cost 4 check); aggiornati verify_focus_ready/verify_focus_produce/verify_focus_costs/verify_china_focus_increase/verify_net_prep/verify_commands al nuovo ordine. Suite motore 138/138.",
	"v0.7.155 - Le Nazioni ESAURITE ora si 'girano' con un'animazione (schiacciamento orizzontale, come un vero giro di carta) e il grigio è molto più marcato (prima troppo simile alle carte pronte) - stessa animazione ora anche per le Growth Card con abilità 'una volta per round' già usate e per le carte Commercio girate. RICONTROLLATE le carte scansionate dell'espansione: le 4 Carte Azione aggiuntive avevano margini bianchi e frammenti della carta vicina (ritagliate male) - ririttagliate pulite dal PDF originale. Trovato e corretto un bug più serio: le 10 Growth Card italiane (Cambio di Leadership, Tenore di Vita Elevato, Autorità Inconfutabile, Programma Nucleare, Ottimizzazione delle Entrate, Affermazione di Predominio, Forza di Reazione Rapida, Collaborazione con gli Alleati, Efficienza delle Risorse, Vantaggio Operativo) mostravano per errore l'arte delle Growth Card BASE inglesi (10 immagini condivise da 2 carte diverse) - ora ognuna ha la sua arte propria, estratta dal PDF originale. +3 test (verify_country_flip, verify_growth_flip, verify_growth_art_unique). Suite motore 138/138.",
	"v0.7.154 - FIX: 2 bug segnalati. 1) Choose Focus - riattivare le Nazioni esaurite non funzionava mai: il comando 'prep_ready_pick'/'prep_ready_skip' mancava nella validazione del command bus (GameCommands.valid_shape) e veniva sempre respinto come 'malformato', quindi toccare le carte esaurite non aveva alcun effetto. 2) Growth 'Industrial Development' ('Aumenta di 1 due delle tue Produzioni') non offriva mai la scelta: l'acquisto di una Growth Card non eseguiva mai l'effetto proprio della carta (solo 'Vantaggio Operativo' aveva un caso dedicato) — ora ogni Growth con un effetto immediato lo risolve subito dopo l'acquisto. Corretta anche la semantica di 'increase_production' con count>1: ora fa scegliere N Produzioni DISTINTE (ognuna +1), come da regolamento, invece di +N su una sola. Controllate tutte le altre 19 Growth Card: nessun altro bug (le abilità 'ongoing' sono tutte collegate; 'Space Program'/'Olympic Games' sono correttamente senza abilità, verificato sull'arte delle carte). +2 test (verify_growth_industrial_dev 7 check). Suite motore 138/138, nessuna regressione sui test Growth/Carte Azione/Orientamenti.",
	"v0.7.153 - ESPANSIONE: ultimi 8 Orientamenti Strategici (Diplomacy & Dominance) — modulo COMPLETO (12 su 12)! Nuovi meccanismi: 'Cambio di Regime' (Russia/USA, prendi 1 Nazione Alleata altrui non iniziale di valore 1), 'BRICS' (Russia, 2 Regioni a scelta poi la Cina può prendere la terza), 'Agenzia Centrale di Intelligence' (USA, scambia 1 tua Influenza temporanea con 1 permanente altrui), 'Aiuti Economici e Militari' (USA, esaurisci 2 alleati in Regioni diverse per +Influenza x2 e +2 Diplomazia), 'Surplus Commerciale' (China, +Influenza in ogni Regione con almeno 35 money di Export), 'Consenso di Pechino' (China, +Prosperità poi Engage anche senza alleati nella Regione), 'Politica Estera di Sicurezza Comune' (UE, esaurisci Stati membri per Produrre Diplomazia+Armata poi Engage o Base). +1 test (verify_strategic_assets_dd2, 41 check). Suite motore 138/138.",
	"v0.7.152 - FEDELTA' BOT (difesa): i Bot ora scelgono dove muovere/costruire Armate seguendo i criteri MINACCIA del regolamento Automa, prima solo semplificati: 1) una Regione della propria zona dove sono SOTTO MINACCIA (minima differenza Minaccia-Difesa), 2) una Regione della zona dove mettono un altro giocatore sotto minaccia, 3) fuori zona dove ne minacciano di più, 4) una valida qualsiasi. Prima i Bot non si difendevano mai. Su 50 simulazioni tutte-Bot il bilanciamento migliora nettamente (scarto-tipo vittorie 10.8 -> 6.4; la Russia, prima a 0 vittorie, ora vince; la Cina non domina più). Nuovo strumento di diagnostica (scomposizione VP per fonte) per misurare il bilanciamento. Suite motore 138/138.",
