...
func _init() -> void:
	...
	# 2) Il client sceglie il Focus -> PRIMA si offre l'Aumento Produzione (sincronizzato a CHI
	#    agisce, il client), NON ancora la Produce del Focus.
	client.apply_command(GameCommands.choose_focus(1, 1, 0))   # Domestic
	await process_frame
	var s2: bool = host._prep_awaiting_increase and client._prep_awaiting_increase and not host._produce_mode
	print("[%s] dopo il Focus: Aumento Produzione offerto e sincronizzato (host=%s, client=%s)" % [
	...
	# 4) Il client salta l'aumento -> ORA si apre la PRODUZIONE del Focus (ultimo passo),
	#    sincronizzata a CHI agisce. La produzione non e' piu' automatica.
	client.apply_command(GameCommands.increase_production(1, 3, ""))
	await process_frame
	var s4: bool = host._produce_mode and client._produce_mode and host._produce_after == "prep" \
		and not host._prep_awaiting_increase
	print("[%s] dopo l'aumento: Produce del Focus aperta e sincronizzata (host=%s, client=%s)" % [
		"OK" if s4 else "FAIL", str(host._produce_mode), str(client._produce_mode)])
	if not s4: fails += 1

	# 5) Il client conferma la produzione (anche a 0) -> la Preparazione avanza (qui finisce:
	#    era l'ultimo giocatore del turn_order, inizia l'Azione).
	client.apply_command(GameCommands.produce(1, 4, {}))
	await process_frame
	var s5: bool = host._ui_phase == "Azione" and client._ui_phase == "Azione"
	print("[%s] dopo la Produce la Preparazione avanza (host fase=%s, client fase=%s)" % [
		"OK" if s5 else "FAIL", host._ui_phase, client._ui_phase])
	if not s5: fails += 1
